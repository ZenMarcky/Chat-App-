    ...
    bcryptjs.hash(password,10).then((hash)=>{
     db.collection('User').insertOne({
        ...
        updatedAt: new Date()
         ...
         const accessToken = sign({username:username,email:email,id:data.insertedId},"fd2efe108eaef7de733d43cbd70e12d2632036755f7b29c30d052d0a0d58a4a3");
  ...
  db.collection('User').find({email:email}).forEach((Data)=>{
        ...
        if(checkUser.email === email){
           ...
           if(!match){
               ...
               const accessToken = sign({username:checkUser.username,email:checkUser.email,id:checkUser._id},"fd2efe108eaef7de733d43cbd70e12d2632036755f7b29c30d052d0a0d58a4a3");
               response.cookie('userToken',accessToken,{
                   maxAge: 1000*60*60,
                   expires: new Date('01 12 2021'),
                   secure: process.env.NODE_ENV === "production" ? true : false,
